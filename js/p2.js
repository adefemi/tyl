...
function createPhysics() {
  ...
  for (var n = 0; n < gameData.ballsArray.length; n++) {
    ballPhysics_arr.push({
      ...
      body: "",
      material: "",
      property: { radius: ballRadius / scaleX / 2 },
      position: [0, 0]
    });
    ...
    ballPhysics_arr[n].body.position[0] =
      (gameData.ballsArray[n].obj.x - canvasW / 2) / scaleX;
    ballPhysics_arr[n].body.position[1] =
      (gameData.ballsArray[n].obj.y - canvasH) / scaleY;
  ...
  for (var n = 0; n < gameData.cageArray.length; n++) {
    ...
    hitPhysics_arr[n].body.position[0] =
      (gameData.cageArray[n].x - canvasW / 2) / scaleX;
    hitPhysics_arr[n].body.position[1] =
      (gameData.cageArray[n].y - canvasH) / scaleY;
  ...
  for (var n = 0; n < holder_arr.length; n++) {
    ...
    holder_arr[n].body.position[1] =
      (gameData.sphereY + 200 - canvasH) / scaleY;
  ...
  world.on("beginContact", function(evt) {
    if (evt.bodyA.contactType == "ball" && evt.bodyB.contactType == "cage") {
      ...
      evt.bodyA.contactType == "cage" &&
      evt.bodyB.contactType == "ball"
    ) {
  ...
  world.on("preSolve", function(evt) {
    for (var i = 0; i < evt.contactEquations.length; i++) {
      ...
      if (eq.bodyA.contactType == "ball" && eq.bodyB.contactType == "cage") {
        ...
        eq.bodyA.contactType == "cage" &&
        eq.bodyB.contactType == "ball"
      ) {
...
function checkCollision(eq) {
  ...
  if (
    eq.bodyA.contactType === "ball" &&
    eq.bodyB.contactType === "cage" &&
    gameData.winArray.indexOf(eq.bodyA.index) != -1
  ) {
    enable = false;
  } else if (
    eq.bodyA.contactType === "ball" &&
    ...
    eq.bodyA.contactType === "cage" &&
    eq.bodyB.contactType === "ball" &&
    gameData.winArray.indexOf(eq.bodyB.index) != -1
  ) {
    enable = false;
  } else if (
    eq.bodyA.contactType === "ball" &&
...
function renderPhysics() {
  for (var n = 0; n < ballPhysics_arr.length; n++) {
    ...
    updateBallRotate(
      n,
      ballPhysics_arr[n].body.velocity[0],
      ballPhysics_arr[n].body.velocity[1],
      ballPhysics_arr[n].body.angle
    );
  }
