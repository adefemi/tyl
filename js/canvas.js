...
function buildGameCanvas() {
  ...
  bgMain = new createjs.Bitmap(loader.getResult("bg"));
  centerReg(bgMain);
  bgMain.x = canvasW / 3.2;
  bgMain.y = canvasH / 2.38;

  itemBallShadow = new createjs.Bitmap(loader.getResult("itemBallShadow"));
  centerReg(itemBallShadow);
  itemBallShadow.x = -500;

  itemBallShadowRed = new createjs.Bitmap(
    loader.getResult("itemBallShadowRed")
  );
  centerReg(itemBallShadowRed);
  itemBallShadowRed.x = -500;

  itemBallShadowBlue = new createjs.Bitmap(
    loader.getResult("itemBallShadowBlue")
  );
  centerReg(itemBallShadowBlue);
  itemBallShadowBlue.x = -500;
    ...
    if (bonusBall) {
      if (n == 0) {
        ...
        scoreString = scoreTextDisplay.replace(
          "[NUMBER]",
          addCommas(score_arr[totalNum - 1])
        );
      } else if (n == 1) {
        textString = bonusTextDisplay.replace("[NUMBER]", totalNum);
        scoreString = scoreTextDisplay.replace(
          "[NUMBER]",
          addCommas(bonusScore[0])
        );
  ...
  gameContainer.addChild(
    ...
    itemBarUser /*ballsSelectContainer*/ /*buttonLucky*/ /*cardContainer*/ /*tableContainer*/
  );
  ...
  canvasContainer.addChild(
    /*bg,*/ mainContainer,
    gameContainer,
    /*resultContainer,*/ confirmContainer,
    optionsContainer,
    buttonSettings,
    guideline
  );
...
function resizeCanvas() {
  if (canvasContainer != undefined) {
    ...
    if (curPage != "game") {
      ...
      // buttonExitrotateHolder.visible = true;
      buttonSoundOn.x = buttonSoundOff.x = buttonSettings.x;
      buttonSoundOn.y = buttonSoundOff.y = buttonSettings.y + distanceNum;
      buttonSoundOn.x = buttonSoundOff.x;
      buttonSoundOn.y = buttonSoundOff.y = buttonSettings.y + distanceNum;
