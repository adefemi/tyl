...
var totalBall = 30; //total balls
var rotateBall = true; //rotate balls in 3d angle
var numberStartZero = true; //number start from zero instead of one
...
function goPage(page) {
  ...
  switch (page) {
    ...
    case "result":
      targetContainer = null; //resultContainer;
      stopGame();
...
function startGame() {
  ...
  for (var n = 0; n < thisTotalBall; n++) {
    ...
    targetBall.scaleX = targetBall.scaleY = 0.9;
...
function readyGame() {
  ...
  var countNum = 0;
  for (var n = 0; n <= totalI; n++) {
    for (var m = 0; m < totalJ; m++) {
  ...
  for (var n = 0; n < thisTotalBall; n++) {
    ...
    createBall(gameData.numberArray[n]);
  }
...
function createBall(number) {
  ...
  var newBallShadow;
  if (refCounter === 0) {
    newBallShadow = itemBallShadow.clone();
    refCounter += 1;
  } else if (refCounter === 1) {
    newBallShadow = itemBallShadowBlue.clone();
    refCounter += 1;
  } else {
    newBallShadow = itemBallShadowRed.clone();
    refCounter = 0;
  }
...
function revealWinNumber() {
  ...
  hightUserSelection(currentNumber);
...
function setRevealBalls() {
  ...
  for (var n = 0; n < score_arr.length; n++) {
    if (n < gameData.winArray.length) {
      ...
      var newGuessBall = gameData.ballsArray[gameData.winArray[n]].obj.clone(
        true
      );
    ...
    if (gameData.winArray.length > score_arr.length) {
      var currentBallRotate =
        gameData.ballsArray[gameData.winArray[gameData.winArray.length - 1]]
          .rotate;
      currentBallRotate.x = currentBallRotate.y = 0;
      var newGuessBall = gameData.ballsArray[
